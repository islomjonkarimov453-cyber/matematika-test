...
function App() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (isExamStarted && !status && document.hidden) {
        alert("Sahifadan chiqish taqiqlangan! Test yakunlandi.");
        handleFinish();
      }
    };
  ...
  const startExam = () => {
    ...
    if (!studentName.trim() || questions.length === 0) return alert("Ism kiriting yoki savollar yo'q!");
  ...
  // --- BITTALAB QO'SHISH FUNKSIYASI ---
  const addSingleQuestion = () => {
    if (!newQuestionText || !newQuestionAnswer) return alert("Savol va javobni to'liq yozing!");
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse">YUKLANMOQDA...</div>;
            ...
            <div className="max-w-xl mx-auto mt-20">
              {!isExamStarted ? (
                <div className={`p-8 rounded-[2.5rem] border shadow-2xl space-y-6 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200'}`}>
                  <h1 className="text-3xl font-black text-center bg-gradient-to-r from-blue-500 to-emerald-500 bg-clip-text text-transparent italic text-shadow">O'QUVCHI TEST</h1>
          ...
          <Route path="/ustoz" element={
            <div className="max-w-6xl mx-auto mt-10 space-y-8 p-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={exportToExcel} className="bg-emerald-600 text-white px-6 py-2 rounded-xl text-xs font-black shadow-lg">NATIJALARNI YUKLASH (EXCEL)</button>
              </div>

              <div className="grid lg:grid-cols-2 gap-8">
                {/* Savol qo'shish qismi */}
                <div className={`p-8 rounded-[2rem] border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200 shadow-lg'}`}>
                  <h2 className="text-sm font-black mb-4 opacity-50 uppercase tracking-widest text-blue-500">Savol Qo'shish (Bittalab yoki Excel)</h2>
                  <div className="space-y-4">
                    ...
                    <button onClick={addSingleQuestion} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black active:scale-95 mb-4 shadow-md">BITTALAB SAQLASH</button>

                    <div className="pt-4 border-t border-white/10">
                      <p className="text-[10px] font-black opacity-50 mb-2">OMMAVIY (EXCEL) YUKLASH:</p>
              ...
              {!isAuthorized ? (
                ...
                <div className="grid lg:grid-cols-5 gap-8">
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Admin ham bittalab qo'sha olishi kerak */}
                    <div className={`p-6 rounded-[2rem] border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white shadow-lg'}`}>
                      <h2 className="text-xs font-black text-emerald-500 uppercase mb-4">Savol Qo'shish (Bittalab/Excel)</h2>
                      <select className="w-full p-2 mb-2 rounded border bg-transparent text-xs" value={targetClass} onChange={e => setTargetClass(e.target.value)}>
                        {['6A', '7A', '7B', '8A', '8B', '9A', '9B', '10A', '10B', '11A', '11B'].map(c => <option key={c} value={c}>{c}-sinf</option>)}
                      </select>
                      <input className="w-full p-3 mb-2 rounded-xl border bg-transparent text-xs" placeholder="Savol..." value={newQuestionText} onChange={e => setNewQuestionText(e.target.value)} />
                      <input className="w-full p-3 mb-4 rounded-xl border bg-transparent text-xs" placeholder="Javob..." value={newQuestionAnswer} onChange={e => setNewQuestionAnswer(e.target.value)} />
                      <button onClick={addSingleQuestion} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-xs mb-4">SAQLASH</button>

                      <p className="text-[9px] font-bold opacity-40 mb-1 uppercase">Ommaviy yuklash:</p>
                      <input type="file" className="text-[10px] block w-full" accept=".xlsx, .xls" onChange={handleExcelUpload} />
                    </div>

                    <button onClick={() => window.confirm("BARCHA NATIJALARNI O'CHIRISH?") && remove(ref(db, 'results'))} className="w-full py-4 bg-red-600 text-white rounded-2xl font-black text-xs shadow-lg">NATIJALARNI TOZALASH</button>
                  </div>
                  ...
                  <div className={`lg:col-span-3 rounded-[2rem] border flex flex-col h-[700px] ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white shadow-xl'}`}>
                    <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
                      <h2 className="font-black text-xs opacity-50 uppercase">Barcha savollar</h2>
                      <button onClick={exportToExcel} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-[10px] font-black italic">NATIJALAR EXCEL</button>
                    </div>
